...
console.log('best', r.best, `(${ms} ms)`); for (const s of r.sequences) console.log('  ', s.hex, s.p.toFixed(3));
for (const rd of r.reads) console.log(`  [${rd.index}]`, S.describe(rd.byte), rd.p.toFixed(2), rd.candidates.slice(1).map(([b, p]) => `| ${S.describe(b)} ${p.toFixed(2)}`).join(' '));
