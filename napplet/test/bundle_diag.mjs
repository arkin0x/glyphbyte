...
let countOk = 0, allOk = 0, n = 0, icon = 0, dots = 0, byteOk = 0, cellsMatched = 0, revOk = 0;
for (const it of items) {
  ...
  for (const rd of r.reads) {
    ...
    if (g.icon === tg.icon) icon++; if (g.dots === tg.dots) dots++; if (rd.byte === t.byte) byteOk++;
  }
...
console.log(`matched cells ${cellsMatched}: icon ${(icon / cellsMatched).toFixed(3)} dots ${(dots / cellsMatched).toFixed(3)} whole byte ${(byteOk / cellsMatched).toFixed(3)}`);
